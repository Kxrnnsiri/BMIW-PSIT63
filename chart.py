...
import os
from screen.workout import workoutSelector
...
def chart():
    ...
    def openWorkout():
        chart.destroy() # close existing window before load new one
        workoutSelector()
    ...
    nextpage = Button(chart, text="Next",bg="#FFFFFF",fg="#660000",font ="consolas 10", command=openWorkout).grid(row=6,column=0)
